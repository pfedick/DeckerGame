...
{
    if (translator)  translator->tr(id);
    return Speech(id);
}
...
Speech Translator::tr(int id) const
{
    Speech speech(id);
    ...
    if (data.exists(path)) {
        ...
        if (node.exists("en/phonetics")) speech.text=node.getString("en/phonetics");
        if (lang.notEmpty() && node.exists(lang) && lang != "en") {
            ...
            if (lang_node.exists("phonetics")) speech.text=lang_node.getString("phonetics");
        }
    } else {
