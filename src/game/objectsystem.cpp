...
void ObjectSystem::loadSpritesets(GPUContext& gpu)
{
	...
	spriteset[Spriteset::GenericObjects]->load(gpu, "res/objects.tex");
	...
	spriteset[Spriteset::Switches]->load(gpu, "res/switches.tex");
...
void ObjectSystem::draw(GPUContext& gpu, const ppl7::grafix::Rect& viewport, const ppl7::grafix::Point& worldcoords, PlaneId plane, Object::Layer layer) const
{
	...
	for (it = visible_object_map[static_cast<int>(plane)].begin();it != visible_object_map[static_cast<int>(plane)].end();++it) {
		...
		if (object->texture != NULL && object->enabled == true && object->visibleAtPlaytime == true && object->myLayer == layer && (object->difficulty_matrix & dm)) {
			object->draw(gpu, coords);
		}
	}
}

static void drawId(GPUContext& gpu, SpriteTexture* spriteset, int x, int y, uint32_t as)
{
	ppl7::String s;
	s.setf("%d", as);
	int w = (int)s.size() * 10;
	...
	for (size_t p = 0;p < s.size();p++) {
		int num = s[p] - 48 + 3;
		spriteset->draw(gpu, x, y, num);
		x += 10;
	}
}

void ObjectSystem::drawEditMode(GPUContext& gpu, const ppl7::grafix::Rect& viewport, const ppl7::grafix::Point& worldcoords, PlaneId plane, Object::Layer layer) const
{
	std::map<uint64_t, Object*>::const_iterator it;
	ppl7::grafix::Point coords(viewport.x1 - worldcoords.x, viewport.y1 - worldcoords.y);
	for (it = visible_object_map[static_cast<int>(plane)].begin();it != visible_object_map[static_cast<int>(plane)].end();++it) {
		...
		if (object->type() == Decker::Objects::Type::Particle) {
			...
			if (object->texture != NULL && object->myLayer == layer) {
				object->drawEditMode(gpu, coords);
				drawId(gpu, light_objects, object->p.x + coords.x, object->p.y + coords.y, object->id);
				if (object->p != object->initial_p && object->spawned == false) drawId(gpu, light_objects, object->initial_p.x + coords.x, object->initial_p.y + coords.y, object->id);
			}
		}
	}
}
...
void ObjectSystem::drawSelectedSpriteOutline(GPUContext& gpu, const ppl7::grafix::Rect& viewport, const ppl7::grafix::Point& worldcoords, int id)
{
	std::map<uint32_t, Object*>::const_iterator it;
	it = object_list.find(id);
	if (it != object_list.end()) {
...
void ObjectSystem::drawPlaceSelection(GPUContext& gpu, const ppl7::grafix::Point& p, int object_type)
{
	...
	if (repr.sprite_set >= 0) {
		...
		if (texture) {
			texture->draw(gpu,
				...
				p.y,
				repr.sprite_no);
			texture->drawOutlines(gpu, p.x, p.y,
				repr.sprite_no, 1.0f);
		}
	}
}
