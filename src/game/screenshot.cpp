...
void Screenshot::save(LightPlaneId lplane, LightPlayerPlaneMatrix pplane, Type type)
{
    if (done_flag) return;
...
void Screenshot::save(Layer layer, Type type)
{
    if (done_flag) return;
    ...
    try {
        ...
        ppl7::grafix::Drawable draw(surface->pixels, 4 * surface->w, surface->w, surface->h, ppl7::grafix::RGBFormat::A8R8G8B8);
