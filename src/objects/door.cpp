...
bool Door::load(const unsigned char *buffer, size_t size)
{
	if (!Object::load(buffer,size)) return false;
	if (size<save_size) return false;
	key_id=ppl7::Peek32(buffer+9);
	frame_type=ppl7::Peek8(buffer+13);
	door_type=ppl7::Peek8(buffer+14);
	int flags=ppl7::Peek8(buffer+15);
	initial_open=false;
	left_sided=false;
