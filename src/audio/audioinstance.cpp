...
size_t AudioInstance::addSamples(size_t num, ppl7::STEREOSAMPLE_FLOAT* buffer, float volume)
{
	if (!sample) return 0;
	if (fade_start > 0.0f) {
	...
	float vol_l = volume * this->volume;
	float vol_r = volume * this->volume;
	if (positional) {
		ppl7::grafix::Point v = GetViewPos();
		float d = ppl7::grafix::Distance(p, v);
		...
		vol_l = (vol_l * ((float)max_distance - d) / (float)max_distance * leftness);
		vol_r = (vol_r * ((float)max_distance - d) / (float)max_distance * rightness);
	...
	if (vol_l < 0.0f) vol_l = 0.0f;
	if (vol_r > 1.0f) vol_r = 1.0f;
	if (vol_r < 0.0f) vol_r = 0.0f;
